...
const localizedRu = {
  ...
  landing: {
    ...
    story: {
      eyebrow: 'Обмен в движении',
      title: 'Книга движется и помнит путь.',
      description:
        'Проследите один физический экземпляр: от добавления в каталог до временного обмена, передачи и возврата.',
      visualLabel: 'Визуализация обмена процедурной книгой',
      stagesLabel: 'Этапы scroll-истории',
      assistiveListLabel: 'Этапы истории Book Exchange',
      progressLabel: 'Текущий этап',
      fallbackTitle: 'История обмена остаётся читаемой.',
      fallbackDescription:
        'Статичная версия сохраняет те же четыре движения книги, когда анимация недоступна.',
      fallbackReasons: {
        reducedMotion:
          'Включено уменьшение движения, поэтому история показана статично.',
        webgl:
          'WebGL недоступен в этом браузере, поэтому показана статичная книжная сцена.',
        weakDevice:
          'Для стабильности на этом устройстве используется облегчённая статичная история.',
        loading: 'Загрузка процедурной книжной сцены.',
        error:
          'Анимированную сцену не удалось загрузить, поэтому показана статичная история.',
        notMounted: 'Анимированная сцена загрузится, когда раздел попадёт в область просмотра.',
      },
      stages: {
        add: {
          title: 'Добавить',
          description:
            'Книга поднимается из каталога и раскрывается, когда владелец добавляет экземпляр на полку.',
        },
        share: {
          title: 'Поделиться',
          description:
            'Временная копия движется к другому читателю, а исходная запись остаётся у владельца.',
        },
        give: {
          title: 'Передать',
          description:
            'Физическая книга переходит к владельцу B, когда обмен становится постоянным.',
        },
        return: {
          title: 'Вернуть',
          description:
            'Книга возвращается к владельцу A, закрывается, и обмен считается завершённым.',
        },
      },
    },
    workflow: {
